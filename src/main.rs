...
fn main() {
    ...
    if args.inject {
        ...
        balatro.replace_file(args.output.unwrap().as_str(), input_bytes).expect("Error while replacing file");
        let inject_duration = inject_start.elapsed();
        green_ln!("Done!");

        if need_cleanup {
            yellow_ln!("Cleaning up...");
            fs::remove_file(args.input.unwrap()).expect("Error while deleting file");
            green_ln!("Done!");
        }
